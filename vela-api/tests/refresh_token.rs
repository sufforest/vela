...
//! - POST /v3/refresh with consumed refresh token returns 401 M_UNKNOWN_TOKEN
...
async fn register_with_refresh(h: &Harness, user: &str, pw: &str) -> serde_json::Value {
    let body = json!({
        "username": user,
        "password": pw,
...
#[tokio::test]
async fn register_with_refresh_returns_paired_tokens() {
    let h = Harness::new();
    let body = register_with_refresh(&h, "elena", "pw").await;
    ...
    // The minted access token must authenticate immediately.
    let access = body["access_token"].as_str().unwrap();
    assert_eq!(whoami(&h, access).await, StatusCode::OK);
}
...
async fn refresh_rotates_and_invalidates_old_access() {
    ...
    // Old refresh token is consumed; reusing it must fail.
    let (status3, body3) = refresh(&h, &refresh1).await;
    assert_eq!(status3, StatusCode::UNAUTHORIZED);
    assert_eq!(body3["errcode"], "M_UNKNOWN_TOKEN");
    assert_eq!(body3["soft_logout"], false);
}

#[tokio::test]
async fn refresh_twice_with_same_token_returns_unknown() {
    let h = Harness::new();
    ...
    assert_eq!(body["soft_logout"], false);
}

#[tokio::test]
async fn refresh_unknown_token_returns_unknown() {
    let h = Harness::new();
